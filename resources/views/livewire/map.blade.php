<div class="w-full h-screen relative">
    ...
    <div id="vehicles" class="absolute top-0 left-0 p-4 z-20 w-48 h-full">
        <div id="card"
            class="bg-white border p-2 border-gray-200 rounded-lg shadow-xl bg-opacity-30 min-h-full backdrop-blur-sm">
            <h2 class="mb-2 text-lg font-semibold text-gray-900 dark:text-white">Vehicles:</h2>
            <ul
                class="text-xs font-medium text-gray-900 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                @foreach($data as $vehicle)
                    <li class="w-full px-4 py-1" onclick="flyToStore(`{{ $vehicle['imei'] }}`)">
